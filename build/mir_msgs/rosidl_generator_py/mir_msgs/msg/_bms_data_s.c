...
bool mir_msgs__msg__bms_data__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // temperature3
    PyObject * field = PyObject_GetAttrString(_pymsg, "temperature3");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->temperature3 = (int32_t)PyLong_AsLong(field);
    Py_DECREF(field);
  }
  {  // cycle_count
...
PyObject * mir_msgs__msg__bms_data__convert_to_py(void * raw_ros_message)
{
  ...
  {  // temperature3
    PyObject * field = NULL;
    field = PyLong_FromLong(ros_message->temperature3);
    {
      int rc = PyObject_SetAttrString(_pymessage, "temperature3", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
